...
def eval(data_handler, model, tokenizer, device):
    """
    :param data_handler: with validation data
    :param model: the model to be evaluated
    :param tokenizer:
    :param device: cpu/cuda
    >>> df = pd.DataFrame({'words': ["To read Alan Turing", "not to read", "testing batch"], 'ner': ["O O B-PER I-PER", "O O O", "O O"]})
    >>> dh = DataHandler(df, batch_size=1)
    >>> tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    >>> model = BertForTokenClassification.from_pretrained("bert-base-uncased", num_labels=dh.get_num_labels())
    >>> res, res_agg, tag_report, f1 = eval(dh, model, tokenizer, torch.device("cpu"))
    >>> type(res)
    <class 'dict'>
    >>> type(res_agg)
    <class 'dict'>
    ...
    for batch in tqdm(data_handler):
        ...
        sent, tags = data
        with torch.no_grad():
            word_tokens, gold_labels = tokenize_with_label_alignment(sent, tags, tokenizer)
            X = torch.LongTensor(tokenizer.convert_tokens_to_ids(word_tokens)).unsqueeze(0)  # batch size = 1
            X = X.to(device)
            # X: [ 1, sent_length + word_pieces ]

            out = model(X)[0]
            # out [batch_size, sent_len + word pieces, num_classes]
            assert (out.shape == torch.Size([len(batch), X.shape[1], data_handler.get_num_labels()])), \
                "unexpected out.shape = {}, expected: {}".format(out.shape, torch.Size([len(batch),
                                                                                        X.shape[1],
                                                                                        data_handler.get_num_labels()]))
            pred_idxs = out.argmax(dim=2).squeeze(0)
            # pred_idxs [ batch_size, sent_len + word_pieces ]
            pred_labels = data_handler.idx2tag(pred_idxs)
            assert (len(gold_labels) == len(pred_labels)), \
                "different length labels: gold: {}, pred: {}, pred_idxs: {}".format(gold_labels, pred_labels, pred_idxs)
            gold.append(gold_labels)
            predicted.append(pred_labels)

    tagset = data_handler.get_labels()
    tagset.remove("O")
    ...
    ner_evaluator = Evaluator(gold, predicted, data_handler.get_labels())
    res, res_agg = ner_evaluator.evaluate()
    return res, res_agg, tagtype_report, tagtype_f1_score
